...
import { fechaLocalDia, formatoDia } from "./lib/date";
...
export default function DiarioComidas() {
  ...
  return <main className="app"><div className="content">
    <header className="topbar"><div><p>{vista === "diario" ? etiquetaDia : "Tu evolución"}</p><h1>{vista === "diario" ? "Tu día" : "Health insights"}</h1></div><span className="avatar" aria-label="Perfil">●</span></header>
    ...
    {cargando ? <p className="empty-state">Cargando registros…</p> : vista === "panel" ? <AnalyticsPanel comidas={comidas} objetivos={OBJETIVOS} /> : <>
  ...
  </div><nav className="bottom-nav" aria-label="Navegación principal"><button className={vista === "diario" ? "active" : ""} onClick={() => setVista("diario")}><b>⌂</b>Hoy</button><button className="scan-button" onClick={() => { setPanelAbierto(true); setMensaje(null); }} aria-label="Registrar comida">＋</button><button className={vista === "panel" ? "active" : ""} onClick={() => setVista("panel")}><b>⌁</b>Progreso</button></nav><NewMealDrawer abierto={panelAbierto} texto={textoJson} mensaje={mensaje} guardando={guardando} onTexto={setTextoJson} onCerrar={() => setPanelAbierto(false)} onGuardar={guardarRegistro} /></main>;
}
